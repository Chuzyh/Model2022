...
在模拟$10^6$次初始情况后，我们发现在\textbf{$20\%$}的偏移误差下，投票机制模型能达到100\%的正确性，而在\textbf{$40\%$}的偏移误差下，仍有\textbf{7\%}的错误率，而再引入一架无人机后，该错误率能降至\textbf{2\%}，符合有效定位的要求。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.35\linewidth]{pic/error area.eps}
  \caption{误差范围示意图}
  \label{误差范围示意图}
  \end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.45\linewidth]{pic/Rplot01.jpeg}
...
结合各轮次信息，对除FY00、FY01外的无人机J，在$\Delta JOP$中即可求得圆心角$\angle JOP$。
...
\subsubsection{二分移动路径法}
在锥形编队队形下，我们可以通过控制其他无人机到$FY01$的连线与$FY01-FY05$这条边的夹角大小，二分确定其他无人机的方向，具体二分方法和问题一的第3小问的方法类似，但由于$FY01$可以作为接收点，我们可以直接得到夹角的角度信息，而不是通过三角形内其他两个角的角度，间接计算夹角。
...
1,\textbf{readme}：
\lstinputlisting[language=C++]{support/readme}

2,\textbf{geometry.h}：
\lstinputlisting[language=C++]{support/code/geometry.h}

3,\textbf{task1.cpp}：
\lstinputlisting[language=C++]{support/code/task1.cpp}

4,\textbf{task2\_simple.cpp}：
\lstinputlisting[language=C++]{support/code/task2_simple.cpp}

5,\textbf{task2\_vote.cpp}：
\lstinputlisting[language=C++]{support/code/task2_vote.cpp}

6,\textbf{task2\_vote2.cpp}：
\lstinputlisting[language=C++]{support/code/task2_vote2.cpp}

7,\textbf{task3\_sample.cpp}：
\lstinputlisting[language=C++]{support/code/task3_sample.cpp}
...
\lstinputlisting[language=C++]{support/code/task3_sim.cpp}
